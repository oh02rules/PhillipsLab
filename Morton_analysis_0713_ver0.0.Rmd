...
To answer the question we perform an ANOVA on a fitted model with three factors, giving F-statistics. As an example, we fit a one-way ANOVA with linear modeling technique (lm function). The summary results give us t-stat. and p-value for each coefficients (species or any nested grouping), where the values are compared to "0".
...
summary(fit4)
```
The ANOVA results show both Phylo and Myco types appear to be statistically significant, while species are not. This may lead us to assume a model where the mean varies among groups (i.e., Phylo and Myco types) performs much better than a model using a single grand mean. In our case particularly, Myco. types may be the best grouping strategy.
...
Because exudation rates that are nested along mycorrhizal types appear to be significantly different, we visualize the difference with a boxplot. Two-way ANOVA involing two or more levels of grouping (Myco, Phylo, and Leaf) with species will come!!
...
First, let’s examine the correlation between root exudation and all root traits (This needs more studies about multivariate linear modeling techinique; more to come!!).
